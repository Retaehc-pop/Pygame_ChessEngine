...
def main():
    ...
    while running:
        human_turn = (game_state.whitemove and player_one) or (
            not game_state.whitemove and player_two)
        ...
        if not game_over and not human_turn and not move_undone:
            if not ai_thinking:
                ai_thinking = True
                return_queue = Queue()
                move_finder_process = Process(target=ChessAI.find_best_move, args=(
...
def highlightSquares(screen, game_state, valid_moves, square_selected):
    ...
    if square_selected != ():
        ...
        if game_state.board[row][col][0] == (
                'w' if game_state.whitemove else 'b'):
            s = p.Surface((SQUARE_SIZE, SQUARE_SIZE))
            s.set_alpha(100)
            s.fill(p.Color('blue'))
